        ...
        if (err) {
            ...
            fs.exists(downloadDir + fileName, function (exists) {
                ...
                if (!exists) {
                    ...
                    }).pipe(imageStream).on('error', function (err) {
                            console.log('error-->' + err + "--dir-->" + downloadDir + fileName);
                            fs.unlink(downloadDir + fileName, function () {
                            });
                            cb(err, null);
                        });
...
exports.pageImgDownload = function (imageDb, downloadDir, cb) {
    ...
    for (var i in imageDb) {
        var imageUrl = imageDb[i];
        var fileName = urlUtil.parse(imageUrl).pathname;
        var imageDir = pathUtil.dirname(downloadDir + fileName);
        var imageFileName = pathUtil.basename(downloadDir + fileName);
        //console.log(imageDir+imageFileName);
        requestHttp(imageUrl, imageDir, imageFileName, cb);
    }
};